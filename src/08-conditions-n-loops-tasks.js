...
function getFizzBuzz(num) {
    ...
    if (num % 5 !== 0 && num % 3 === 0) return 'Fizz';
    return num;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
    let leftBoard = '';
    let rightBoard = '';
    if (isStartIncluded === true) leftBoard = '[';
    else leftBoard = '(';
    if (isEndIncluded === true) rightBoard = ']';
    else rightBoard = ')';
    if (a < b) return `${leftBoard}${a}, ${b}${rightBoard}`;
    return `${leftBoard}${b}, ${a}${rightBoard}`;
}
...
function isCreditCardNumber(ccn) {
    let sum = 0;
    const curNum = ccn.toString();
    const curNumLength = curNum.length;
    for (let i = 1; i <= curNumLength; i += 1) {
        let some = Number(curNum[curNumLength - i]);
        if (i % 2 === 0) { some *= 2; }
        if (some > 9) { some -= 9; }
        sum += some;
    }
...
function isBracketsBalanced(str) {
    const bracketSequence = str.split('');
    ...
    while (index < bracketSequence.length) {
        const lastOfBracketSequence = stack[stack.length - 1];
...
function getMatrixProduct(m1, m2) {
    const firstRows = m1.length;
    const firstCols = m1[0].length;
    const secondRows = m2.length;
    const secondCols = m2[0].length;
    const finalMatrix = new Array(firstRows).fill(0).map(() => new Array(secondCols).fill(0));
    for (let i = 0; i < firstRows; i += 1) {
        for (let j = 0; j < secondCols; j += 1) {
            for (let k = 0; k < firstCols; k += 1) {
                finalMatrix[i][j] += m1[i][k] * m2[k][j];
            }
        }
    }
    return finalMatrix;
